...
export default function Professors() {
  ...
  const [professorsPerPage, setProfessorsPerPage] = useState(12);
  const [totalPages, setTotalPages] = useState(1);

  // Add a toast message state and function if not already present
  const [toastMessage, setToastMessage] = useState(null);

  // Add state to store the count data
  const [countData, setCountData] = useState({ pagination: { total: 0 } });

  // Add state for departments
  const [departments, setDepartments] = useState([]); 

  // Display professors count - using memoization to avoid render-phase calculations
  const ProfessorCount = () => {
    // Use useMemo to cache the calculation results
    const displayInfo = useMemo(() => {
      // Fetch data directly from state updated by API response
      const totalDisplayed = professors.length; // Professors on the current page
      const totalFilteredCount = countData?.pagination?.total || 0; // Total professors matching filters

      // Check if any filters are applied
      const isFiltered = searchTerm || departmentFilter || ratingFilter > 0;
    ...
    }, [professors, countData, searchTerm, departmentFilter, ratingFilter]); // Dependencies updated

    return (
      <div className="bg-indigo-900/40 backdrop-blur-sm px-5 py-4 rounded-xl text-center mb-6 border-2 border-indigo-500/50 shadow-lg">
        <div className="flex items-center justify-center mb-1">
          ...
          <span className="text-indigo-100 font-semibold text-sm">PROFESSOR COUNT</span>
        </div>
        <span className="text-indigo-100 font-medium text-lg">
          {/* Show "Showing X of Y" only when there are multiple pages */}
          {totalPages > 1 && `Showing ${displayInfo.totalDisplayed} of `}

          {/* Total professors count matching filters */}
          {displayInfo.totalFilteredCount} professors

          {/* Filter descriptions */}
          {searchTerm && ` matching "${searchTerm}"`}
          {departmentFilter && ` in "${departmentFilter}"`}
          {ratingFilter > 0 && ` rated ${ratingFilter}+`}
  ...
  async function fetchProfessors(page = currentPage, filters = { searchTerm, departmentFilter, ratingFilter }) {
    try {
      setLoading(true);
      setError(null); // Clear previous errors

      // Construct query parameters including pagination and filters
      const params = new URLSearchParams({
        page: page.toString(),
        pageSize: professorsPerPage.toString(),
      });
      ...
      // API should return professors and pagination info reflecting filters
      if (data.professors && Array.isArray(data.professors) && data.pagination) {
        setProfessors(data.professors);
        // We need to set filteredProfessors to the same data, since the UI relies on this state
        setFilteredProfessors(data.professors);

        // Update count data and total pages based on the API response (which reflects filters)
        setCountData(data); // Assuming data structure is { professors: [], pagination: { total: ..., page: ..., totalPages: ... } }
        ...
        if (data.professors.length > 0) {
          const uniqueDepartments = [...new Set(data.professors.map(p => p.metadata.department).filter(Boolean))];
        ...
        setFilteredProfessors([]); // Also clear filteredProfessors
        setCountData({ pagination: { total: 0 } });
        setTotalPages(1);
      }
    } catch (err) {
      console.error("Error fetching professors:", err); // Log the full error
  ...
  // Update useEffect to refetch when page or filters change
  useEffect(() => {
    // Only refetch if this effect runs after filters have changed
    if (currentPage !== undefined) { // This check ensures we're not in the initial render
      // Reset to page 1 whenever filters change, otherwise fetch current page
      const isFilterChange = searchTerm || departmentFilter || ratingFilter > 0;
      const pageToFetch = isFilterChange ? 1 : currentPage;
      if(isFilterChange) {
        setCurrentPage(1); // Reset page state when filters change
      }
      fetchProfessors(pageToFetch, { 
        searchTerm: searchTerm, 
        departmentFilter: departmentFilter, 
        ratingFilter: ratingFilter 
      });
    }
  }, [currentPage, professorsPerPage, searchTerm, departmentFilter, ratingFilter]);
  ...
  // Get the current page of professors
  const getCurrentProfessors = () => {
    // Since we've moved filtering to the server side,
    // we should simply return professors which are already properly paginated
    return professors;
  };
  ...
  const PaginationControls = () => {
    ...
    // Calculate page buttons to show (current, +/- 1 page, first, last)
    const pageButtons = [];
    ...
    if (startPage <= 2) {
      endPage = Math.min(totalPages - 1, maxPageButtonsToShow - 1);
    }
    
    if (endPage >= totalPages - 1) {
      startPage = Math.max(2, totalPages - maxPageButtonsToShow + 2);
    }
            ...
            <button
              ...
              className={`h-9 w-9 flex items-center justify-center rounded-lg ${
                currentPage === page 
                  ...
                  : 'bg-indigo-600/50 text-white hover:bg-indigo-700'
              }`}
            >
              {page}
            </button>
      ...
      <div className="container mx-auto px-4 py-20">
        ...
        <div className="mb-8">
                ...
                <div className="w-full md:w-auto">
                  ...
                  >
                    ...
                    {departments.map((department, index) => (
                      <option key={index} value={department} className="bg-gray-800 text-white">
                        {department}
                      </option>
                    ))}
                  </select>
          ...
          <div className="text-indigo-200 text-sm mt-2">
            {professors.length} {professors.length === 1 ? 'professor' : 'professors'} {searchTerm ? 'matching your search' : 'displayed'}
            {countData?.pagination?.total && professors.length !== countData.pagination.total && ` (out of ${countData.pagination.total} total)`}
          </div>
        </div>
        
        {/* Display professors count */}
        <ProfessorCount />
        
        {professors.length > 0 ? (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {getCurrentProfessors().map((professor, index) => (
                <ProfessorCard
                  key={professor.id}
                  professor={professor}
                  onClick={() => setSelectedProfessor(professor)}
                  onCompare={() => toggleCompare(professor)}
